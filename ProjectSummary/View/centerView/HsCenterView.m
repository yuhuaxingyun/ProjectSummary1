...
- (void)setup {
    ...
    _oldContentFrame = CGRectMake(0, 0, self.frame.size.width, 30);
    _oldLabelFrame = CGRectMake(5, 5, _oldContentFrame.size.height-10, _oldContentFrame.size.height-10);
    ...
    UITapGestureRecognizer *centerGesture = [[UITapGestureRecognizer alloc]initWithTarget:self action:@selector(contentClick)];
    [self.contentView addGestureRecognizer:centerGesture];
...
- (void)setStyle {
    ...
    self.timeLabel.font = [UIFont systemFontOfSize:7.0f];
...
- (void)reset:(NSNumber *)animal {
    ...
    void(^block)(void) = ^{
        ...
        self.usedButton.titleLabel.font = [UIFont boldSystemFontOfSize:14];
        [self.usedButton setTitle:@"点击用车叫啥都是" forState:UIControlStateNormal];
